<?php

function cek_auth()
{

    $ci = get_instance();
    if (!$ci->session->userdata('username')) {
        redirect('login');
    } else {
        $role_id = $ci->session->userdata('role_id');
        $menu = $ci->uri->segment(1);

        $queryMenu = $ci->db->get_where('user_menu', ['menu' => $menu])->row_array();
        $menu_id = $queryMenu['id'];

        $userAccessMenu = $ci->db->get_where('user_access_menu', [
            'role_id' => $role_id,
            'menu_id' => $menu_id
        ]);

        if ($userAccessMenu->num_rows() < 1) {
            $sub_menu = $ci->uri->segment(1);
            $querySubMenu = $ci->db->get_where('user_sub_menu', ['sub_url' => $sub_menu])->row_array();
            $sub_menu_id = $querySubMenu['id_sub'];

            $userAccessSubMenu = $ci->db->get_where('user_access_sub_menu', [
                'sub_role_id' => $role_id,
                'sub_menu_id' => $sub_menu_id
            ]);
            if ($userAccessSubMenu->num_rows() < 1) {
                redirect('welcome');
            }
        }
    }

    // function cek_akses($role_id, $menu_id)
    // {

    //     $ci = get_instance();

    //     $ci->db->where('role_id', $role_id);
    //     $ci->db->where('menu_id', $menu_id);
    //     $result = $ci->db->get('user_access_menu');

    //     if ($result->num_rows() > 0) {
    //         redirect('welcome');
    //     }
    // }
}
